...
echo "== 1. Agent-visible semantic context =="
cargo run -q -p skardi-cli --no-default-features -- \
  query --ctx "$CTX" --semantics "$SEMANTICS" --schema --all

echo "== 2. Unsafe operations are rejected before serving =="
...
echo "== 3. Run the safe diagnostic endpoint =="
...
for _ in $(seq 1 60); do
  if curl --fail --silent "http://127.0.0.1:$PORT/data_source" >"$DEMO_DIR/catalog.json"; then
    break
  fi
  sleep 0.5
done
...
curl --fail --silent --show-error \
  -X POST "http://127.0.0.1:$PORT/diagnose-failed-payments/execute" \
...
if ! grep -Fq 'Acme Robotics' "$DEMO_DIR/diagnosis.json" || ! grep -Fq 'issuer_declined_after_3ds' "$DEMO_DIR/diagnosis.json"; then
  echo "Diagnostic endpoint did not return the expected staging result." >&2
