...
- The README hero is a four-variant contract (English/Chinese × light/dark): its centre mascot must remain the smiling blinking GIF while surrounding layout changes. Run `bash scripts/check-readme-hero.sh` after modifying `README*.md` or `asset/controlled-db-debugging-hero*`.
